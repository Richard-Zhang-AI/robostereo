...
_DEFAULT_OUTPUT = _GS_HEAD_ROOT / "output_gs_from_xyz_video.mp4"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Render GS video for all frames using external xyz depth and DA3 GS head."
    )
    parser.add_argument(
        "--dataset-dir",
        type=Path,
        required=True,
        help="Directory with rgb.mp4, geometry.safetensors, intrinsics.txt, extrinsics.txt",
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    if not cap.isOpened():
        raise RuntimeError(f"Failed to open {rgb_video}")

    total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
    print(f"[INFO] Loading model (device={args.device})...", flush=True)
    ...
    num_frames = min(num_frames, xyz.shape[0], intr_all.shape[0], extr_all.shape[0])
    print(f"[INFO] Model loaded. Processing {num_frames} frames...", flush=True)
    ...
    for i in range(num_frames):
        ...
        # preprocess image through DA3 input_processor to get size
        ...
        with torch.inference_mode():
            ...
            # Apply fixed view rotation in camera frame (c2w' = c2w @ R)
            if args.view_yaw != 0.0 or args.view_pitch != 0.0 or args.view_roll != 0.0:
            ...
            # Render single view with render_3dgs (same as gs_from_xyz_depth / export_to_gs_image)
            ex = as_homogeneous(ex_t).float().to(gaussians.means)
            in_tensor = in_t.float().to(gaussians.means)
            ex = ex.reshape(-1, 4, 4)[:1]
            in_tensor = in_tensor.reshape(-1, 3, 3)[:1]
            intr_normed = in_tensor.clone()
            intr_normed[..., 0, :] /= W
            intr_normed[..., 1, :] /= H
            color, _ = render_3dgs(
                extrinsics=ex,
            ...
            frame = color[0].clamp(0, 1).permute(1, 2, 0).cpu().numpy()
            frame = (frame * 255).astype(np.uint8)

        if writer is None:
            args.output.parent.mkdir(parents=True, exist_ok=True)
            writer = cv2.VideoWriter(
                str(args.output), cv2.VideoWriter_fourcc(*"mp4v"), 10, (W, H)
            )
        writer.write(cv2.cvtColor(frame, cv2.COLOR_RGB2BGR))
        print(f"[INFO] Rendered frame {i+1}/{num_frames}", flush=True)

    if writer is not None:
        writer.release()
    cap.release()
    print(f"\n[DONE] saved video to: {args.output}")
    return 0
