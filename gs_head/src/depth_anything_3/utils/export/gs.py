...
from depth_anything_3.model.utils.gs_renderer import render_3dgs, run_renderer_in_chunk_w_trj_mode
...
def export_to_gs_image(
    prediction: Prediction,
    export_dir: str,
    extrinsics: Optional[torch.Tensor] = None,  # world2cam, "b v 4 4"
    intrinsics: Optional[torch.Tensor] = None,  # unnormed intrinsics, "b v 3 3"
    out_image_hw: Optional[tuple[int, int]] = None,
    output_name: str = "gs_render",
    save_depth: bool = True,
) -> None:
    """Render a single view from input camera and save as PNG. For single-image use."""
    import imageio
    ...
    tgt_extrs = (
        ...
        else torch.from_numpy(prediction.extrinsics).unsqueeze(0).to(gs_world.means)
    )
    tgt_intrs = (
        intrinsics
    ...
    ex = as_homogeneous(tgt_extrs).float().to(gs_world.means)
    in_t = tgt_intrs.float().to(gs_world.means)
    ex = ex.reshape(-1, 4, 4)[:1]
    in_t = in_t.reshape(-1, 3, 3)[:1]
    ...
    os.makedirs(os.path.join(export_dir, "gs_image"), exist_ok=True)
    rgb = (color[0].clamp(0, 1) * 255).byte().permute(1, 2, 0).cpu().numpy()
    imageio.imwrite(
        os.path.join(export_dir, "gs_image", f"{output_name}.png"),
        rgb,
        quality=95,
    )
    if save_depth:
        depth_vis = vis_depth_map_tensor(depth)
        depth_img = (depth_vis[0].clamp(0, 1) * 255).byte().permute(1, 2, 0).cpu().numpy()
        imageio.imwrite(
            os.path.join(export_dir, "gs_image", f"{output_name}_depth.png"),
            depth_img,
            quality=95,
        )
