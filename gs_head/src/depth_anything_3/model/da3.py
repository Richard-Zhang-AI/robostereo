...
class DepthAnything3Net(nn.Module):
    ...
    def forward(
        ...
        print("---------------------------")
        print("feats:", len(feats))  # 4
        print("feats[0]:", len(feats[0]))  # 2
        print("x.shape:", x.shape)  # x.shape: torch.Size([1, n, 3, 280, 504]),n为输入的图像的数量

        # Process features through depth head
        with torch.autocast(device_type=x.device.type, enabled=False):
            output = self._process_depth_head(feats, H, W)
            # print("output:", output)
            print("output.depth:", output["depth"].shape)
            print("output.depth_conf:", output["depth_conf"].shape)
            print("output.ray:", output["ray"].shape)
            print("output.ray_conf:", output["ray_conf"].shape)
            '''
            output.depth: torch.Size([1, 3, 378, 504])
            output.depth_conf: torch.Size([1, 3, 378, 504])
            output.ray: torch.Size([1, 3, 216, 288, 6])
            output.ray_conf: torch.Size([1, 3, 216, 288])
            '''

            if use_ray_pose:
                output = self._process_ray_pose_estimation(output, H, W)
                # print("output_ray:", output)
            else:
...
class NestedDepthAnything3Net(nn.Module):
    ...
    def forward(
        ...
        # print("metric_output:", metric_output.shape)
