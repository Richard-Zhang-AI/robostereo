#!/usr/bin/env python3
"""
RoboStereo TTPA - VLA Sim Eval (Conda side, sync with Docker Cosmos)
Adapted from WMPO/OEPL.

Runs in Conda: VLA generates actions -> save to shared dir -> pause ->
Docker reads actions, runs Cosmos, signals continue -> Conda receives, executes actions, continues.

Usage (Conda env, start first):
...
def main() -> None:
    parser = argparse.ArgumentParser(description="VLA Sim Eval - Conda side (sync with Docker)")
    ...
    parser.add_argument("--wait-timeout", type=float, default=3600, help="Timeout (sec) for continue signal")
    parser.add_argument(
        "--gripper-postproc",
    ...
    parser.add_argument("--max-resamples", type=int, default=5, help="Max VLA resamples per chunk when Docker signals resample")
    ...
    for ep in range(args.runs):
        ...
        for chunk_idx in range(args.num_chunks):
            ...
            print(f"[Conda][Ep {ep:02d}][Chunk {chunk_idx:03d}] VLA generating actions...", flush=True)
            ...
            # Write to shared dir, signal ready, wait for Docker (continue or resample)
            ...
            print(f"[Conda][Ep {ep:02d}][Chunk {chunk_idx:03d}] Actions saved, waiting for Docker signal...", flush=True)

            resample_count = 0
            while True:
                signal = conda_wait_for_signal(args.sync_dir, timeout_sec=args.wait_timeout)
                if signal is None:
                    print(f"[Conda] Wait timeout, exiting", flush=True)
                    break
                if signal == "continue":
                    break
                # resample: re-run VLA, get new actions, write again
                resample_count += 1
                if resample_count >= args.max_resamples:
                    print(f"[Conda][Ep {ep:02d}][Chunk {chunk_idx:03d}] Max resamples ({args.max_resamples}) reached, continuing with current actions", flush=True)
                    break
                print(f"[Conda][Ep {ep:02d}][Chunk {chunk_idx:03d}] Resample signal received, re-sampling VLA (attempt {resample_count + 1})...", flush=True)
                actions_chunk = get_vla_action(
                    cfg_ns, vla, processor, policy_obs, task_description, action_head, None
                )
                raw_actions = np.asarray(actions_chunk)
                ...
                actions_list = actions_for_sim.astype(float).tolist()
                conda_write_and_signal(args.sync_dir, vla_image, actions_list, meta)
                print(f"[Conda][Ep {ep:02d}][Chunk {chunk_idx:03d}] New actions saved, waiting for Docker signal...", flush=True)

            if signal is None:
                break

            print(f"[Conda][Ep {ep:02d}][Chunk {chunk_idx:03d}] Continue signal received, executing actions...", flush=True)

            # Execute actions in sim
