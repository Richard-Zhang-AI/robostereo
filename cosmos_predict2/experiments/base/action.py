...
from cosmos_predict2._src.predict2.action.configs.action_conditioned.data import (
    build_dataloader,
    bridge_13frame_480_640_train_dual_dataset,
    bridge_13frame_480_640_val_dual_dataset,
)
    ...
    dict(
        ...
        model=dict(
            config=dict(
                ...
                state_t=1 + 12 // 4,
                tokenizer=dict(
                    vae_pth="/workspace/checkpoints/wan2pt2_vae.pth",  #!!!!!
                    s3_credential_path="",
                ),
                net=dict(
                    action_dim=7,
...
"""
torchrun --nproc_per_node=1 --master_port=12341 -m scripts.train --config=cosmos_predict2/_src/predict2/action/configs/action_conditioned/config.py  -- experiment=ac_reason_embeddings_rectified_flow_2b_256_320_dual
"""
ac_reason_embeddings_rectified_flow_2b_256_320_dual = LazyDict(
    dict(
        defaults=[
            DEFAULT_CHECKPOINT.experiment,
            {"override /model": "action_conditioned_video2world_dual_fsdp_rectified_flow"},
            {"override /net": "cosmos_v1_2B_action_conditioned"},
            {"override /conditioner": "action_conditioned_video_conditioner"},
            {"override /data_train": "bridge_13frame_480_640_train_dual"},
            {"override /data_val": "bridge_13frame_480_640_val_dual"},
            "_self_",
        ],
        job=dict(
            project="cosmos_predict2_action_conditioned",
            group="cosmos_predict_v2p5",
            name="2b_bridge_action_conditioned_dual",
        ),
        video_source="rgb",
        optimizer=dict(
            lr=2 ** (-14.5),
        ...
        dataloader_train=L(build_dataloader)(
            dataset=bridge_13frame_480_640_train_dual_dataset,
            batch_size=1,
            drop_last=True,
        ),
        dataloader_val=L(build_dataloader)(
...
cs.store(
    group="experiment",
    package="_global_",
    name="ac_reason_embeddings_rectified_flow_2b_256_320_dual",
    node=ac_reason_embeddings_rectified_flow_2b_256_320_dual,
)
