...
class ActionConditionedInferenceArguments(CommonInferenceArguments):
    ...
    visual_condition_source: str = "rgb"
    """Visual condition source: "rgb" or "geometry"."""
    geometry_normalize: bool = True
    """Whether to apply geometry min-max normalization (only used when visual_condition_source == "geometry")."""
    geometry_min: float | None = None
    """Min XYZ value for geometry min-max normalization (None reads from training config)."""
    geometry_max: float | None = None
    """Max XYZ value for geometry min-max normalization (None reads from training config)."""
