...
def load_model_state_dict_from_checkpoint(
    ...
    def _align_tensor_to_target(value, target):
    ...
    def _normalize_state_dict_for_model(state_dict):
        ...
        for key, value in state_dict.items():
            ...
            normalized_value = _align_tensor_to_target(value, target)
            normalized_state_dict[key] = normalized_value
        ...
        if converted_keys:
            ...
            log.warning(
                f"Aligned {len(converted_keys)} checkpoint tensors between Tensor/DTensor formats "
                f"before load_state_dict ({preview}{suffix})"
            )
        return normalized_state_dict

    def _unwrap_pt_payload(payload):
        if not isinstance(payload, dict):
            return payload
        if "model" in payload and isinstance(payload["model"], dict):
            log.info("Detected wrapped pt checkpoint payload; loading payload['model'].")
            return payload["model"]
        if "state_dict" in payload and isinstance(payload["state_dict"], dict):
            log.info("Detected wrapped pt checkpoint payload; loading payload['state_dict'].")
    ...
    if load_from_local:
        ...
        if distributed.is_rank0():
            ...
            if checkpoint_format == "dcp" and os.path.isdir(local_s3_ckpt_fp):
                ...
                remap_ema_to_reg = False
                try:
                    _model_wrapper = ModelWrapper(model, load_ema_to_reg=load_ema_to_reg)
                except AssertionError:
                    remap_ema_to_reg = load_ema_to_reg
                ...
                if remap_ema_to_reg:
                    ...
                    for key in list(_state_dict.keys()):
                        if key.startswith("rgb.net."):
                            remapped_keys[key] = "rgb.net_ema." + key[len("rgb.net.") :]
                        elif key.startswith("xyz.net."):
                            remapped_keys[key] = "xyz.net_ema." + key[len("xyz.net.") :]
                        elif key.startswith("net."):
                            remapped_keys[key] = "net_ema." + key[len("net.") :]
                ...
                if remap_ema_to_reg:
                    ...
                    for key, value in _state_dict.items():
                        if key.startswith("rgb.net_ema."):
                            ...
                            remapped[key] = value
                    _state_dict = remapped
                _model_wrapper.load_state_dict(_state_dict)
                local_state_dict = None
            else:
                local_state_dict = easy_io.load(local_s3_ckpt_fp, weights_only=INTERNAL)
                if checkpoint_format == "pt":
                    local_state_dict = _unwrap_pt_payload(local_state_dict)
                    local_state_dict = _normalize_state_dict_for_model(local_state_dict)
        ...
        if checkpoint_format == "dcp":
            ...
            if distributed.is_rank0():
                ...
                model_state = _normalize_state_dict_for_model(model_state)
