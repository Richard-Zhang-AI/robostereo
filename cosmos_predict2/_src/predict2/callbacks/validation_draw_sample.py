...
class ValidationDrawSample(Callback):
    ...
    @torch.no_grad()
    def x0_pred(self, model, data_batch, iteration, tower_name: str):
        tag = "ema" if self.is_ema else "reg"
        if tower_name != "main":
            tag = f"{tag}_{tower_name}"
        ...
        local_path = self.run_save(to_show, batch_size, iteration, "x0", tag_suffix=tower_name)
    ...
    def run_sample(self, trainer, model, data_batch, iteration, should_save):
        ...
        for tower_name, tower_model, tower_batch in self._iter_towers(model, data_batch):
            tag = "ema" if self.is_ema else "reg"
            if tower_name != "main":
                tag = f"{tag}_{tower_name}"
            ...
            if self.rank == 0 and wandb.run:
                data_type = "image" if tower_model.is_image_batch(tower_batch) else "video"
                tag += f"_{data_type}"
                info = {
                ...
                if self.do_x0_prediction:
                    ...
                    mse_loss = mse_loss.tolist()
                    info.update(
                        {f"x0_pred_mse_{tag}/Sigma{sigmas[i]:0.5f}": mse_loss[i] for i in range(len(mse_loss))}
                    )
    ...
    def sample(self, model, data_batch, iteration, should_save, tower_name: str):
        tag = "ema" if self.is_ema else "reg"
        if tower_name != "main":
            tag = f"{tag}_{tower_name}"
        ...
        batch_size = x0.shape[0]
        if should_save:
            local_path = self.run_save(to_show, batch_size, iteration, "sample", tag_suffix=tower_name)
            return local_path
        return None

    def run_save(self, to_show, batch_size, iteration, save_name, tag_suffix: str = "main") -> Optional[str]:
        ...
        save_path = os.path.join(str(iteration), str(self.sample_counter), f"{tag}_{save_name}")
    ...
    def _iter_towers(self, model, data_batch):
        if (
            isinstance(data_batch, dict)
            and "rgb" in data_batch
            and "xyz" in data_batch
            and hasattr(model, "rgb_model")
        ):
            return [
                ("rgb", model.rgb_model, data_batch["rgb"]),
                ("xyz", model.xyz_model, data_batch["xyz"]),
            ]
        return [("main", model, data_batch)]
