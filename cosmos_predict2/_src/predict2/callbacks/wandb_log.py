...
class WandbCallback(Callback):
    ...
    def __init__(
        ...
        self.train_rgb_log = _LossRecord()
        self.train_xyz_log = _LossRecord()
    ...
    def on_training_step_end(
        ...
        if not skip_update_due_to_unstable_loss:
            edm_loss = output_batch.get("edm_loss", loss).detach().float()
            if model.is_image_batch(data_batch):
                self.train_image_log.loss += loss.detach().float()
                self.train_image_log.iter_count += 1
                self.train_image_log.edm_loss += edm_loss
            else:
                self.train_video_log.loss += loss.detach().float()
                self.train_video_log.iter_count += 1
                self.train_video_log.edm_loss += edm_loss

            self.final_loss_log.loss += loss.detach().float()
            self.final_loss_log.iter_count += 1
            self.final_loss_log.edm_loss += edm_loss

            if "rgb" in output_batch and "xyz" in output_batch:
                rgb_loss = output_batch.get("rgb_loss", loss).detach().float()
                xyz_loss = output_batch.get("xyz_loss", loss).detach().float()
                rgb_edm = output_batch["rgb"].get("edm_loss", rgb_loss).detach().float()
                xyz_edm = output_batch["xyz"].get("edm_loss", xyz_loss).detach().float()
                self.train_rgb_log.loss += rgb_loss
                self.train_rgb_log.edm_loss += rgb_edm
                self.train_rgb_log.iter_count += 1
                self.train_xyz_log.loss += xyz_loss
                self.train_xyz_log.edm_loss += xyz_edm
                self.train_xyz_log.iter_count += 1
        else:
            if model.is_image_batch(data_batch):
        ...
        if iteration % (self.config.trainer.logging_iter * self.logging_iter_multipler) == 0:
            ...
            avg_rgb_loss, avg_rgb_edm_loss = self.train_rgb_log.get_stat()
            avg_xyz_loss, avg_xyz_edm_loss = self.train_xyz_log.get_stat()
                ...
                info.update(
                    {
                        ...
                        f"train{self.wandb_extra_tag}/rgb_loss": avg_rgb_loss,
                        f"train{self.wandb_extra_tag}/rgb_edm_loss": avg_rgb_edm_loss,
                        f"train{self.wandb_extra_tag}/xyz_loss": avg_xyz_loss,
                        f"train{self.wandb_extra_tag}/xyz_edm_loss": avg_xyz_edm_loss,
