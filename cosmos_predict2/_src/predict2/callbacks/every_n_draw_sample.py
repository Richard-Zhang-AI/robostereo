...
class EveryNDrawSample(EveryN):
    ...
    @torch.no_grad()
    def x0_pred(self, trainer, model, data_batch, output_batch, loss, iteration, tower_name: str):
    ...
    def sample(self, trainer, model, data_batch, output_batch, loss, iteration, tower_name: str):
        tag = "ema" if self.is_ema else "reg"
        if tower_name != "main":
            tag = f"{tag}_{tower_name}"
