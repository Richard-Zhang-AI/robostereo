...
from cosmos_predict2._src.predict2.action.models.action_conditioned_video2world_dual_rectified_flow_model import (
    DualActionConditionedConfig,
    DualActionVideo2WorldModelRectifiedFlow,
)
...
FSDP_DUAL_RECTIFIED_FLOW_CONFIG = dict(
    trainer=dict(
        distributed_parallelism="fsdp",
    ),
    model=L(DualActionVideo2WorldModelRectifiedFlow)(
        config=DualActionConditionedConfig(
            fsdp_shard_size=8,
            state_t=24,
        ),
        _recursive_=False,
...
def register_model():
    ...
    cs.store(
        group="model",
        package="_global_",
        name="action_conditioned_video2world_dual_fsdp_rectified_flow",
        node=FSDP_DUAL_RECTIFIED_FLOW_CONFIG,
    )
