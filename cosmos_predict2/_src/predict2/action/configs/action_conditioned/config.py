...
class Config(config.Config):
    ...
    # Dataset video source: "rgb" or "geometry"
    video_source: str = "rgb"
    # Whether to apply geometry min-max normalization in training (only used when video_source == "geometry").
    geometry_normalize: bool = True
    # 这两个参数是训练数据中对geometry进行的归一化操作。
    geometry_min: float = -0.55   
    geometry_max: float = 1.51
    # Root directory for Bridge-style action dataset.
    data_root: str = "datasets/bridge"
