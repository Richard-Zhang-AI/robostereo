...
class TextEncoder:
    ...
    def compute_text_embeddings_online(
        ...
        # Keep model resident on the target device to avoid repeated .to() overhead.
        if getattr(self, "_resident_device", None) != self.device:
            self.model = self.model.to(self.device)
            self._resident_device = self.device
            try:
                param_device = next(self.model.parameters()).device
            except Exception:
                param_device = "unknown"
            print(f"[MEM][Cosmos][text_encoder] moved model to {self.device}, param_device={param_device}")
        elif not getattr(self, "_mem_logged", False):
            try:
                param_device = next(self.model.parameters()).device
            except Exception:
                param_device = "unknown"
            print(f"[MEM][Cosmos][text_encoder] model already on {self.device}, param_device={param_device}")
