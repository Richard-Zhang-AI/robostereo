...
class Video2WorldInference:
    ...
    def _get_data_batch_input(
        ...
        Args:
            ...
            extrinsics: (torch.Tensor, optional) Raw camera extrinsics, required for multiview camera conditioned model.
            intrinsics: (torch.Tensor, optional) Raw camera intrinsics, required for multiview camera conditioned model.
            image_size: (torch.Tensor, optional) Image size (height, width), required for multiview camera conditioned model.
        ...
        # Debug logging for extrinsics and intrinsics
        log.info(f"DEBUG _get_data_batch_input: extrinsics is None: {extrinsics is None}")
        if extrinsics is not None:
            log.info(f"DEBUG _get_data_batch_input: extrinsics shape: {extrinsics.shape}, dim: {extrinsics.dim()}")
        log.info(f"DEBUG _get_data_batch_input: intrinsics is None: {intrinsics is None}")
        if intrinsics is not None:
            log.info(f"DEBUG _get_data_batch_input: intrinsics shape: {intrinsics.shape}, dim: {intrinsics.dim()}")
        ...
        # Add extrinsics and intrinsics if provided (for multiview camera conditioned models)
        if extrinsics is not None:
            data_batch["extrinsics"] = extrinsics
            log.info(f"DEBUG _get_data_batch_input: Added extrinsics to data_batch with shape: {data_batch['extrinsics'].shape}")
        if intrinsics is not None:
            data_batch["intrinsics"] = intrinsics
            log.info(f"DEBUG _get_data_batch_input: Added intrinsics to data_batch with shape: {data_batch['intrinsics'].shape}")
        if image_size is not None:
            data_batch["image_size"] = image_size
            log.info(f"DEBUG _get_data_batch_input: Added image_size to data_batch with shape: {data_batch['image_size'].shape}")
        ...
        # Debug logging after GPU conversion
        if "extrinsics" in data_batch and data_batch["extrinsics"] is not None:
            log.info(f"DEBUG _get_data_batch_input: After GPU conversion, extrinsics shape: {data_batch['extrinsics'].shape}, dim: {data_batch['extrinsics'].dim()}")
        if "intrinsics" in data_batch and data_batch["intrinsics"] is not None:
            log.info(f"DEBUG _get_data_batch_input: After GPU conversion, intrinsics shape: {data_batch['intrinsics'].shape}, dim: {data_batch['intrinsics'].dim()}")
    ...
    def generate_vid2world(
        ...
        intrinsics: torch.Tensor | None = None,
        image_size: torch.Tensor | None = None,
        extract_layer_ids: list[int] | None = None,
        extract_at_steps: str | list[int] | None = None,
    ):
        """
        ...
        Args:
            ...
            extrinsics: Raw camera extrinsics, required for multiview camera conditioned model.
            intrinsics: Raw camera intrinsics, required for multiview camera conditioned model.
            extract_layer_ids: List of layer indices to extract features from (e.g., [4, 12, 20]). If None, no features extracted.
            extract_at_steps: When to extract features:
                - None: extract from all steps (default)
                - 'first': extract only from the first step
                - 'last': extract only from the last step
                - [i, j, ...]: extract only from specific step indices (0-based)

        Returns:
            torch.Tensor: The generated video tensor (B, C, T, H, W) in the range [-1, 1].
            tuple: (features, step_info) if extract_layer_ids is not None, otherwise just the video tensor.
        """
        ...
        # Add feature extraction parameters if specified
        if extract_layer_ids is not None:
            extra_kwargs["extract_layer_ids"] = extract_layer_ids
            extra_kwargs["extract_at_steps"] = extract_at_steps
            log.info(f"Feature extraction enabled: layers={extract_layer_ids}, steps={extract_at_steps}")
        ...
        # Check if the model supports feature extraction (returns 3 values)
        # We need to check if the method signature has been updated
        import inspect
        sig = inspect.signature(generate_samples)
        has_feature_extraction = len(sig.parameters) > 0 and 'kwargs' in sig.parameters

        if extract_layer_ids is not None:
            # Feature extraction requested - expect 3 return values
            sample_list, all_timesteps_features, all_timesteps_step_info = generate_samples(
                data_batch,
            ...
            sample = sample_list
            log.info(f"Successfully extracted features from {len(all_timesteps_features)} timesteps")
            for i, info in enumerate(all_timesteps_step_info):
                log.info(f"  Timestep {i}: step_idx={info['step_idx']}, timestep={info['timestep']}, num_features={len(all_timesteps_features[i])}")
        else:
            # No feature extraction - expect 1 return value (backward compatible)
            ...
            all_timesteps_features = None
            all_timesteps_step_info = None
        ...
        # Return video only or video with features
        if extract_layer_ids is not None:
            return video, all_timesteps_features, all_timesteps_step_info
        else:
            return video
