...
class DistributedCheckpointer(AbstractCheckpointer):
    ...
    def load(
        ...
        if checkpoint_path is not None:
            if not easy_io.exists(f"{checkpoint_path}", backend_key=self.load_s3_backend_key):
                log.warning(f"Checkpoint not found, skipping resume: {checkpoint_path}")
                checkpoint_path = None
            else:
    ...
    def save(
        ...
        self._maybe_save_dual_tower_weights(model, iteration)
    ...
    def _maybe_save_dual_tower_weights(self, model: ImaginaireModel, iteration: int) -> None:
        if not distributed.is_rank0():
            return
        if not (hasattr(model, "rgb_model") and hasattr(model, "xyz_model")):
            return
        checkpoint_dir = f"{self.save_dirname}/iter_{iteration:09d}"
        for tower_name in ("rgb", "xyz"):
            tower_model = getattr(model, f"{tower_name}_model", None)
            if tower_model is None:
                continue
            output_path = f"{checkpoint_dir}/{tower_name}_model_ema_bf16.pt"
            log.info(f"Saving {tower_name} tower checkpoint to {output_path}")
            easy_io.dump(tower_model.state_dict(), output_path)
