...
class ImaginaireTrainer:
    ...
    def train(
        ...
        debug_dataloader = os.getenv("COSMOS_DATALOADER_DEBUG", "0") == "1"
        debug_every = max(1, int(os.getenv("COSMOS_DATALOADER_DEBUG_EVERY", "1")))
        debug_first = max(0, int(os.getenv("COSMOS_DATALOADER_DEBUG_FIRST", "1")))
        if debug_dataloader and distributed.is_rank0():
            log.info(
                "Dataloader debug enabled: COSMOS_DATALOADER_DEBUG=1 "
                f"first={debug_first} every={debug_every}"
            )
            try:
                dl_len = len(dataloader_train)
            except Exception as exc:
                dl_len = f"error: {exc}"
            ds = getattr(dataloader_train, "dataset", None)
            sampler = getattr(dataloader_train, "sampler", None)
            try:
                ds_len = len(ds) if ds is not None else "n/a"
            except Exception as exc:
                ds_len = f"error: {exc}"
            try:
                sampler_len = len(sampler) if sampler is not None else "n/a"
            except Exception as exc:
                sampler_len = f"error: {exc}"
            log.info(
                "Dataloader summary: "
                f"type={type(dataloader_train).__name__} len={dl_len} "
                f"dataset={type(ds).__name__ if ds is not None else None} ds_len={ds_len} "
                f"sampler={type(sampler).__name__ if sampler is not None else None} sampler_len={sampler_len}"
            )
            if hasattr(sampler, "num_replicas") or hasattr(sampler, "rank"):
                log.info(
                    "Sampler details: "
                    f"num_replicas={getattr(sampler, 'num_replicas', None)} "
                    f"rank={getattr(sampler, 'rank', None)} "
                    f"num_samples={getattr(sampler, 'num_samples', None)} "
                    f"total_size={getattr(sampler, 'total_size', None)} "
                    f"drop_last={getattr(sampler, 'drop_last', None)}"
                )
        if distributed.is_rank0():
            try:
                if len(dataloader_train) == 0:
                    raise ValueError(
                        "Dataloader has zero batches. Check dataset paths and sampler settings "
                        "(e.g., drop_last with empty sampler)."
                    )
            except TypeError:
                # Some dataloaders may not implement __len__.
                pass
        ...
        with (
            ...
            batch_index = 0
            while True:
                if debug_dataloader and distributed.is_rank0() and (
                    batch_index < debug_first or batch_index % debug_every == 0
                ):
                    log.info("Dataloader: creating iterator (epoch loop).")
                    ...
                    try:
                        with (
                            ...
                            if debug_dataloader and distributed.is_rank0() and (
                                ...
                                log.info(f"Dataloader: next() start (batch_index={batch_index}).")
                            ...
                            if debug_dataloader and distributed.is_rank0() and (
                                ...
                                log.info(f"Dataloader: next() done in {dt:.3f}s (batch_index={batch_index}).")
                    except StopIteration:
                        if batch_index == 0:
                            log.error(
                                "Dataloader produced no batches (StopIteration at first batch). "
                                "Check dataset paths and sampler settings."
                            )
                            _end_training = True
                        if debug_dataloader and distributed.is_rank0() and (
                            batch_index < debug_first or batch_index % debug_every == 0
                        ):
                            log.info(f"Dataloader: StopIteration (batch_index={batch_index}).")
                        break
                    finally:
