...
class Checkpointer:
    ...
    def __init__(self, config_checkpoint: CheckpointConfig, config_job: JobConfig, callbacks: callback.CallBackGroup):
        ...
        self.job_dir_local = config_job.path_local
        ...
        self.save_path = config_checkpoint.save_path
        self.save_path_rgb = config_checkpoint.save_path_rgb
        self.save_path_xyz = config_checkpoint.save_path_xyz
    ...
    def save(
        ...
        if distributed.get_rank() == 0:
            ...
            # Run the checkpoint saver in a separate thread (training state).
            ...
            # Also save weight-only checkpoints to explicit paths (if configured).
            self._save_weight_files(model, iteration, checkpoint_file)
    ...
    def _resolve_save_path(self, path: str, filename: str) -> str:
        candidate = Path(path)
        if not candidate.is_absolute():
            candidate = Path(self.job_dir_local) / candidate
        if candidate.suffix == ".pt":
            # Treat as filename prefix, append iteration-aware filename.
            return str(candidate.with_name(f"{candidate.stem}_{filename}"))
        return str(candidate / filename)

    def _save_weights_local(self, state_dict: dict[str, torch.Tensor], checkpoint_path: str) -> None:
        try:
            os.makedirs(os.path.dirname(checkpoint_path), exist_ok=True)
            torch.save(state_dict, checkpoint_path)
            log.success(f"Saved weights: {checkpoint_path}")
        except Exception as e:  # noqa: BLE001
            log.exception(f"Weights failed to save: {e}")

    def _save_weight_files(self, model: ImaginaireModel, iteration: int, checkpoint_file: str) -> None:
        save_path = self.save_path.strip() if self.save_path else ""
        if not save_path:
            return

        base_model = model.module if hasattr(model, "module") else model
        # Always keep a single full-model PT file for dual/single tower models.
        candidate = Path(save_path)
        if not candidate.is_absolute():
            candidate = Path(self.job_dir_local) / candidate
        resolved = str(candidate if candidate.suffix == ".pt" else (candidate / "model.pt"))
        weights = {"model": base_model.state_dict(), "iteration": iteration}
        weights = misc.to(weights, device="cpu")
        self._save_weights_local(weights, resolved)
...
def non_strict_load_model(model: torch.nn.Module, checkpoint_state_dict: dict) -> _IncompatibleKeys:
    ...
    if skipped_extra_state_keys:
        preview = ", ".join(skipped_extra_state_keys[:4])
        suffix = "" if len(skipped_extra_state_keys) <= 4 else ", ..."
        log.warning(
            "Skipping "
            f"{len(skipped_extra_state_keys)} TransformerEngine FP8 _extra_state keys during checkpoint load "
            f"({preview}{suffix})"
        )
