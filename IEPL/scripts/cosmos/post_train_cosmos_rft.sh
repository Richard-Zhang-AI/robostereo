#!/bin/bash
# ============================================================
# IEPL (RoboStereo) - Cosmos-RFT Training with MimicGen Data
# Adapted from VLA-RFT/OEPL.
#
# Uses local verl worker + Cosmos world model for GRPO training.
#
# Expected Docker volume layout (RoboStereo):
#   /workspace              → RoboStereo root (cosmos_predict2, checkpoints, etc.)
#   /workspace/OEPL         → OEPL (checkpoints, datasets, verl)
#   /workspace/IEPL         → IEPL (this project)
# ============================================================
...
# IEPL_ROOT: this project root (legacy env name VLA_RFT_ROOT for compatibility)
VLA_RFT_ROOT="${VLA_RFT_ROOT:-/workspace/IEPL}"
...
# ---- Ray: increase keepalive timeout for long Cosmos video generation ----
# Cosmos 22 chunks ~2min; default Ray gRPC keepalive may timeout. Set to 5min.
export RAY_GRPC_KEEPALIVE_TIMEOUT_MS="${RAY_GRPC_KEEPALIVE_TIMEOUT_MS:-300000}"
...
EXPERIMENT_NAME="${TASK_NAME}_cosmos_rft"
...
# Clean stale model caches
# echo "Clearing transformers module cache..."
...
DEFAULT_COSMOS_CKPT="$(cd "$VLA_RFT_ROOT/.." && pwd)/checkpoints_files/coffee_action_conditioned/checkpoints-RGB/model_ema_fp32.pt"
COSMOS_CHECKPOINT_PATH="${COSMOS_CHECKPOINT_PATH:-$DEFAULT_COSMOS_CKPT}"
[ -n "${COSMOS_CHECKPOINT_PATH}" ] && [ ! -f "$COSMOS_CHECKPOINT_PATH" ] && COSMOS_CHECKPOINT_PATH="$DEFAULT_COSMOS_CKPT"
...
export ALIGN_PATH="${ALIGN_PATH:-$REPO_ROOT/align.json}"
...
export RUNTIME_ENV_PATH="$RUN_DIR/runtime_env.json"
python - <<'PY'
import json, os
...
data["env_vars"]["RAY_GRPC_KEEPALIVE_TIMEOUT_MS"] = os.environ.get("RAY_GRPC_KEEPALIVE_TIMEOUT_MS", "300000")
with open(out_path, "w", encoding="utf-8") as f:
    json.dump(data, f, indent=2, ensure_ascii=True)
print(f"[runtime_env] wrote {out_path} with VLA_RFT_DEBUG_LOG={debug_log}")
PY
# No global redirection; keep logs in terminal.
echo "Run directory: $RUN_DIR"

# ---- Launch Training ----
cd "$VLA_RFT_ROOT"

# Verify Cosmos checkpoint exists before launching
if [ ! -f "$COSMOS_CHECKPOINT_PATH" ]; then
    echo "ERROR: Cosmos checkpoint not found: $COSMOS_CHECKPOINT_PATH"
    echo "Set COSMOS_CHECKPOINT_PATH to your checkpoint, e.g.:"
    echo "  export COSMOS_CHECKPOINT_PATH=checkpoints_files/coffee_action_conditioned/checkpoints-RGB/model_ema_fp32.pt"
    exit 1
fi
echo "Using Cosmos checkpoint: $COSMOS_CHECKPOINT_PATH"

# Ensure colossalai is installed (required by fsdp_workers)
# Use python -m pip so Ray workers inherit the same env
# python -m pip install colossalai>=0.4.0 -q

# data.state_path is ignored when data.video_root is set
HYDRA_FULL_ERROR=1 python -m iepl_cosmos.main_cosmos_rft \
    ...
    trainer.generate_rpc_min_samples_per_rank=2 \
