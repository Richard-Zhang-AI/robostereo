...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(self,
        ...
        def _safe_load_component(component_name: str, module: torch.nn.Module) -> None:
            try:
                component_path = find_checkpoint_file(ckpt_path, component_name)
                state_dict = load_component_state_dict(component_path)
                module.load_state_dict(state_dict)
            except (AssertionError, FileNotFoundError, OSError) as exc:
                logger.warning(
                    "Missing %s checkpoint under %s; using randomly initialized weights. (%s)",
                    component_name,
                    ckpt_path,
                    exc,
                )

        # Token Generation Mode: No action_head, proprio_projector, etc.
        # Use model.generate() directly for action prediction
        ...
        if self.rank == 0:
            ...
            if self.action_head is not None:
                print_model_size(self.action_head,'action_head')
        ...
        if role == 'actor' and optim_config is not None:
            ...
            # —— Parameter grouping —— #
            use_token_gen = (self.action_head is None)
            if use_token_gen:
                # Token Gen mode: only optimize actor_module (VLA)
                actor_params = [p for p in actor_module_fsdp.parameters() if p.requires_grad]
                if self.rank == 0:
                    print(f"# actor params (Token Gen): {sum(p.numel() for p in actor_params):,}")
                param_groups = [
                    {"params": actor_params, "lr": base_lr, "weight_decay": wd},
                ]
                ...
                actor_lr_scheduler = LambdaLR(actor_optimizer, lr_lambda=[warmup_factor])
            else:
                # Flow Matching mode: action_head + projectors + sigma_net
                ...
                param_groups = [
                    {"params": head_params + proj_params, "lr": base_lr, "weight_decay": wd},
                    {"params": sigma_params, "lr": sigma_lr, "weight_decay": sigma_wd},
                ]
                actor_optimizer = optim.AdamW(param_groups, betas=betas)
                total_steps = _oget(optim_config, "total_training_steps", 0)
                num_warmup_steps = _oget(optim_config, "lr_warmup_steps", -1)
    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        if rollout_name == 'hf':
            ...
            rollout_cfg = OmegaConf.create(OmegaConf.to_container(self.config.rollout, resolve=True))
            with open_dict(rollout_cfg):
                data_cfg = getattr(self.config, 'data', None)
                rollout_cfg.action_bins = int(getattr(data_cfg, 'bc_action_bins', 256) if data_cfg else 256)
                rollout_cfg.action_min = float(getattr(data_cfg, 'bc_action_min', -1.0) if data_cfg else -1.0)
                rollout_cfg.action_max = float(getattr(data_cfg, 'bc_action_max', 1.0) if data_cfg else 1.0)
            # Pass tokenizer separately: OmegaConf does not support non-primitive types (e.g. LlamaTokenizerFast)
            _tokenizer = getattr(self.processor, 'tokenizer', self.processor)
            rollout = HFRollout(module=self.actor_module_fsdp, config=rollout_cfg,
                                tokenizer=_tokenizer,
                                action_head=None, proprio_projector=None,
                                noisy_action_projector=None, sigma_net=None,
                                context_projector=None)
...
class TokenizerWorker(Worker):
    ...
    def process(self, data: DataProto):
        ...
        first_frame = pixels[:, 0:1]                                            # (B, 1, C, H, W)
