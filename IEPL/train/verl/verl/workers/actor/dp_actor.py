...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _forward_micro_batch_token_gen(self, micro_batch, return_entropy: bool = False, return_hidden_states: bool = False) -> Tuple[torch.Tensor, ...]:
        """Token Generation: compute log_probs from token logits."""
        from verl.utils.torch_functional import logprobs_from_logits

        input_ids = micro_batch['input_ids']
        response_tokens = micro_batch['response_tokens']
        attention_mask = micro_batch['attention_mask']
        pixels = micro_batch['pixels']
        B = input_ids.size(0)
        device = input_ids.device

        full_ids = torch.cat([input_ids, response_tokens], dim=1)
        full_attn = torch.cat([
            attention_mask,
            torch.ones((B, response_tokens.size(1)), dtype=attention_mask.dtype, device=device)
        ], dim=1)
        ...
        logits = out.logits[:, -(response_tokens.size(1)+1):-1, :].float()
        log_probs = logprobs_from_logits(logits=logits, labels=response_tokens)
        traj_log_prob = log_probs.sum(dim=1)

        if return_entropy:
            probs = torch.softmax(logits, dim=-1)
            # log_probs should be (B, L); probs is (B, L, V). Ensure compatible shapes.
            if log_probs.dim() == 3 and log_probs.shape[-1] != probs.shape[-1]:
                # Mismatch: use entropy_from_logits on logits instead (avoids shape bug)
                entropy = verl_F.entropy_from_logits(logits).sum(dim=1)
            else:
                log_probs_exp = log_probs.exp().unsqueeze(-1) if log_probs.dim() == 2 else log_probs.exp()
                entropy = -(probs * log_probs_exp).sum(dim=-1).sum(dim=1)
            if return_hidden_states:
                return traj_log_prob, entropy, out.hidden_states
            return traj_log_prob, entropy
        if return_hidden_states:
            return traj_log_prob, None, out.hidden_states
    ...
    def _forward_micro_batch(self, micro_batch, return_entropy: bool = False, return_hidden_states: bool = False) -> Tuple[torch.Tensor, ...]:
        ...
        Returns:
            log_probs: (B,)  # Sum of token log_probs for Token Gen, or flow-matching logp
            entropy:   (B,)  (if return_entropy is True)
        """
        if self.action_head is None:
            return self._forward_micro_batch_token_gen(micro_batch, return_entropy, return_hidden_states)
    ...
    def compute_log_prob(self, data: DataProto) -> torch.Tensor:
        ...
        if self.action_head is None:
            select_keys = ['input_ids', 'attention_mask', 'labels', 'pixels', 'proprio', 'response_tokens']
        else:
            select_keys = [
                'x_chain',
    ...
    def update_policy(self, data: DataProto):
        ...
        # breakpoint()
        if self.action_head is None:
            select_keys = ['advantages', 'attention_mask', 'input_ids', 'labels', 'old_log_probs',
                           'pixels', 'predicted_actions', 'proprio', 'response_tokens']
        else:
        ...
        if self.action_head is not None and (self.config.use_mse_loss or self.config.log_mse_loss):
