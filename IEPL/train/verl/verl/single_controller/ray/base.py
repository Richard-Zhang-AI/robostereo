    ...
    def _init_with_resource_pool(self, resource_pool, ray_cls_with_init, bin_pack, detached):
        ...
        for pg_idx, pg in enumerate(sort_placement_group_by_node_ip(pgs)):
            ...
            for local_rank in range(local_world_size):
                ...
                if rank == 0:
                    # Force actor to start (runs __new__ which creates register_center)
                    try:
                        if hasattr(worker, 'ping'):
                            ray.get(worker.ping.remote(), timeout=60)
                    except Exception as e:
                        pass  # Continue to wait loop; ping may fail if actor not ready yet
                    register_center_actor = None
                    for _ in range(180):  # 180s timeout for slow GPU / Cosmos startup
                    ...
                    assert register_center_actor is not None, (
                        ...
                        f"after 180s. Worker may have crashed during startup. "
