...
class RLDSBatchTransform:
    ...
    def __call__(self, rlds_batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.use_minivla:
            ...
            future_actions_string = self.action_tokenizer(future_actions,self.use_minivla)
            current_action_string = self.action_tokenizer(current_action,self.use_minivla)
            ...
            future_actions_string = ''.join(self.action_tokenizer(future_actions,self.use_minivla))
...
class RLDSBatchTransform_V1:
    ...
    def __call__(self, rlds_batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.use_minivla:
            ...
            future_actions_string = ''.join(self.action_tokenizer(future_actions, use_minivla=False))

            # Get action chunk string
            current_action_string = self.action_tokenizer(current_action, use_minivla=False)
            action_chunk_string = current_action_string + future_actions_string
            action_chunk_len = len(action_chunk_string)
        ...
        return_dict = dict(pixel_values=pixel_values, input_ids=input_ids, labels=labels, 
                           dataset_name=dataset_name, actions=actions )
        # breakpoint()
