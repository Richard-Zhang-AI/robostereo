...
class FlowMatchingActionHead(nn.Module):
    ...
    def predict_flow(self, actions_hidden_states, noisy_actions=None, timestep_embeddings=None, 
        ...
        if noisy_actions is not None:
            noisy_actions_flat = noisy_actions.reshape(batch_size, -1).unsqueeze(-1).to(torch.bfloat16)
            noise_actions_hidden_states = noisy_action_projector(noisy_actions_flat)
        else:
        ...
        # proprio is now already projected to 896-dim, or None
        if proprio is None:
            proprio_features = torch.zeros((batch_size, 896), dtype=torch.bfloat16, device=device)
        else:
            # proprio is (B, 896) already
            proprio_features = proprio.to(dtype=torch.bfloat16, device=device)
