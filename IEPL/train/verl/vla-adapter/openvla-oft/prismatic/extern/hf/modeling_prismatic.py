...
class PrismaticForConditionalGeneration(PrismaticPreTrainedModel):
    ...
    def _replace_input_embeddings(self, input_embeddings, all_actions_mask, noisy_action_features):
        ...
        # Check if mask has any True values (safety check)
        if noisy_action_features.shape[1] == 0 or all_actions_mask.sum() == 0:
            return new_input_embeddings

        num_action_features = noisy_action_features.shape[1]
        mask_counts = all_actions_mask.sum(dim=1)
        effective_count = int(min(num_action_features, mask_counts.min().item()))

        if effective_count == 0:
            return new_input_embeddings

        truncated_features = noisy_action_features[:, :effective_count]

        if (mask_counts > effective_count).any():
            clamped_mask = all_actions_mask.clone()
            for b in range(all_actions_mask.shape[0]):
                if mask_counts[b] > effective_count:
                    true_indices = torch.where(clamped_mask[b])[0]
                    clamped_mask[b, true_indices[effective_count:]] = False
            all_actions_mask = clamped_mask

        repositioned_noisy_action_features = torch.zeros_like(input_embeddings)

        batch_indices = torch.arange(input_embeddings.shape[0], device=input_embeddings.device)
        batch_indices = batch_indices.unsqueeze(1).expand(-1, effective_count)

        masked_indices = torch.stack([torch.where(mask)[0][:effective_count] for mask in all_actions_mask])

        repositioned_noisy_action_features[batch_indices, masked_indices] = truncated_features
    ...
    def forward(
        ...
        # === Handle Multimodal Forward (Token Gen only: prompt + image -> LM) ===
        elif (input_ids.shape[0] == pixel_values.shape[0]) or (inputs_embeds.shape[0] == pixel_values.shape[0]):
            assert past_key_values is None, "Unexpected key `past_key_values` provided during multimodal forward!"

            # Token Gen: input_embeddings = embed(prompt), no action mask / replacement logic
            input_embeddings = self.get_input_embeddings()(input_ids)  # (B, seq_len, D)

            # Vision: pixel_values -> patch embeddings
            projected_patch_embeddings = self._process_vision_features(pixel_values, None, use_film=False)

            # Add proprioceptive state if provided (v2 only)
            if self.version != 'v1' and proprio_projector is not None and proprio is not None:
                projected_patch_embeddings = self._process_proprio_features(
                    projected_patch_embeddings, proprio, proprio_projector
                )

            # [Diffusion] Add diffusion timestep embedding if provided (v2 only)
            if diffusion_timestep_embeddings is not None and self.version != 'v1':
