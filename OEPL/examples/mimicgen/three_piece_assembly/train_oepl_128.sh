...
EXPERIMENT_NAME='three_piece_assembly_wmpo_128' 

TASK_NAME="three_piece_assembly"
UNNORM_KEY="$TASK_NAME"_d0_300_demos

SFT_MODEL_PATH="./checkpoint_files/SFT_models/three_piece_assembly"
REWARD_MODEL_PATH="./checkpoint_files/reward_models/videomae_three_piece_assembly.pth"

rm "$SFT_MODEL_PATH"/config.json.back*
rm "$SFT_MODEL_PATH"/modeling_prismatic.py.back*

cp ./checkpoint_files/modeling_prismatic.py "$SFT_MODEL_PATH/"
cp ./checkpoint_files/preprocessor_config.json "$SFT_MODEL_PATH/"
cp ./checkpoint_files/processing_prismatic.py "$SFT_MODEL_PATH/"
...
HYDRA_FULL_ERROR=1 python -m verl.trainer.main_ppo \
    ...
    actor_rollout_ref.wm.inference_config_path=./checkpoint_files/world_models/three_piece_assembly/P_128/inference_cfg.py \
    actor_rollout_ref.wm.train_config_path=./checkpoint_files/world_models/three_piece_assembly/P_128/train_mimicgen_cfg.py \
    +actor_rollout_ref.wm.update_wm=False \
    actor_rollout_ref.wm.reward_model_path=$REWARD_MODEL_PATH \
    actor_rollout_ref.wm.rm_threshold=0.62 \
