...
def sync_vla_hf_files(pretrained_checkpoint: str, variant: str = "openvla_oft") -> None:
    """
    Copy all local HuggingFace dynamic-module helper files for the given VLA variant
    into the checkpoint directory so `trust_remote_code=True` can resolve relative imports.
    """
    if not os.path.isdir(pretrained_checkpoint):
        return

    repo_root = Path(__file__).resolve().parents[2]
    variant_dir = repo_root / "verl" / "utils" / "vla_utils" / variant
    if not variant_dir.is_dir():
        print(f"WARNING: VLA helper directory not found: {variant_dir}")
        return

    checkpoint_dir = Path(pretrained_checkpoint)
    for src in sorted(variant_dir.glob("*.py")):
        dst = checkpoint_dir / src.name
        _handle_file_sync(str(src), str(dst), src.name)
...
def update_auto_map(pretrained_checkpoint: str) -> None:
    ...
    """
    # Lazy import to avoid torchvision dependency when not needed.
...
def check_model_logic_mismatch(pretrained_checkpoint: str) -> None:
    ...
    # Fallback to repo-local VLA utility directories when ./prismatic is absent.
    for variant in ("openvla_oft", "openvla"):
        variant_dir = repo_root / "verl" / "utils" / "vla_utils" / variant
        if not variant_dir.is_dir():
            continue
        for filename in curr_files.keys():
            if curr_files[filename] is None:
                candidate = variant_dir / filename
                if candidate.exists():
                    curr_files[filename] = str(candidate)
