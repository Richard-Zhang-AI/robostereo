...
from verl.utils.openvla_utils import update_auto_map , check_model_logic_mismatch, sync_vla_hf_files
from verl.utils.cosmos_utils import load_cosmos_infer, read_negative_prompt
...
class RobWMActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(self,
        ...
        if self.config.model.vla == "openvla-oft":
            ...
            if self.rank == 0:
                sync_vla_hf_files(local_path, variant="openvla_oft")
            ...
            AutoModelForVision2Seq.register(OpenVLAConfig, OpenVLAForActionPrediction)
            if self.rank == 0:
                sync_vla_hf_files(local_path, variant="openvla")
                update_auto_map(local_path)
                check_model_logic_mismatch(local_path)
            torch.distributed.barrier()
    ...
    def _build_world_model(self):
        wm_backend = str(self.config.wm.get("backend", "opensora")).lower()
        update_wm = self.config.wm.get('update_wm', False)
        if wm_backend == "cosmos":
            if update_wm:
                raise NotImplementedError("Cosmos backend currently supports inference-only world models (update_wm=False).")

            device = torch.cuda.current_device()
            cosmos_infer = load_cosmos_infer(
                cosmos_root=self.config.wm.cosmos_root,
                experiment=self.config.wm.cosmos_experiment,
                checkpoint_path=self.config.wm.get("cosmos_checkpoint_path", None),
                model_key=self.config.wm.get("cosmos_model_key", None),
                config_file=self.config.wm.cosmos_config_file,
            )
            feature_extractor = VideoMAEFeatureExtractor.from_pretrained(
                "MCG-NJU/videomae-base",
                size=self.config.wm.rm_img_size,
            )
            ...
            return {
                "backend": "cosmos",
                "cosmos_infer": cosmos_infer,
                "cosmos_resolution": str(self.config.wm.get("cosmos_resolution", "256,320")),
                "cosmos_guidance": float(self.config.wm.get("cosmos_guidance", 0.0)),
                "cosmos_seed": int(self.config.wm.get("cosmos_seed", 0)),
                "cosmos_action_chunk_size": int(self.config.wm.get("cosmos_action_chunk_size", 12)),
                "cosmos_action_scale": float(self.config.wm.get("cosmos_action_scale", 20.0)),
                "cosmos_gripper_scale": float(self.config.wm.get("cosmos_gripper_scale", 1.0)),
                "use_normalized_actions": bool(self.config.wm.get("use_normalized_actions", False)),
                "negative_prompt": read_negative_prompt(
                    self.config.wm.get("cosmos_negative_prompt_file", None),
                    default=str(self.config.wm.get("cosmos_negative_prompt", "")),
                ),
                "preview_save_dir": self.config.wm.get("preview_save_dir", None),
                "preview_save_fps": int(self.config.wm.get("preview_save_fps", 6)),
                "rm_model": rm_model,
                "feature_extractor": feature_extractor,
                "rm_threshold": self.config.wm.rm_threshold,
            }

        if not OPEN_SORA_AVAILABLE:
            raise ModuleNotFoundError(
                "opensora is required for the OpenSora world-model backend, but it could not be imported. "
                f"Original import error: {OPEN_SORA_IMPORT_ERROR}"
            )
