    ...
    with torch.no_grad():
        ...
        for idx in id2score:
            if len(id2score[idx]) == 1:
                ...
                group_scores = torch.stack(id2score[idx]).to(device=scores.device, dtype=scores.dtype)
                id2mean[idx] = torch.mean(group_scores)
                id2std[idx] = torch.std(group_scores, unbiased=False)
                if id2std[idx].abs() < epsilon:
                    zero_std_groups += 1
                total_groups += 1
            else:
                raise ValueError(f"no score in prompt index: {idx}")
        if total_groups > 0 and zero_std_groups == total_groups:
            print(
                "[WARN][GRPO] all prompt groups have zero within-group reward std; "
