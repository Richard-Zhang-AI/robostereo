...
class RayTrainer(object):
    ...
    def __init__(self,
        ...
        self.print_metrics = os.environ.get("WMPO_PRINT_METRICS", "0") == "1"
        self.print_metrics_every = int(os.environ.get("WMPO_PRINT_METRICS_EVERY", "1"))
    ...
    def _maybe_print_metrics(self, metrics: dict, global_steps: int) -> None:
        if not self.print_metrics:
            return
        if self.print_metrics_every <= 0:
            return
        if global_steps % self.print_metrics_every != 0:
            return
        reward_items = {k: v for k, v in metrics.items() if k.startswith("train_reward/")}
        loss_items = {k: v for k, v in metrics.items() if "loss" in k}
        if not reward_items and not loss_items:
            return
        parts = []
        for k in sorted(reward_items.keys()):
            try:
                parts.append(f"{k}={float(reward_items[k]):.4f}")
            except Exception:
                parts.append(f"{k}={reward_items[k]}")
        for k in sorted(loss_items.keys()):
            try:
                parts.append(f"{k}={float(loss_items[k]):.4f}")
            except Exception:
                parts.append(f"{k}={loss_items[k]}")
        print(f"[Metrics] step {global_steps}: " + " | ".join(parts))

    def _is_retryable_generate_error(self, exc: Exception) -> bool:
        msg = f"{type(exc).__name__}: {exc}"
        return (
            "ActorUnavailableError" in msg
            or "keepalive watchdog timeout" in msg
            or "temporarily unavailable" in msg
        )

    def _generate_sequences_with_retry(self, prompts: DataProto, tag: str):
        max_retries = int(self.config.trainer.get("ray_generate_max_retries", 2))
        backoff_s = float(self.config.trainer.get("ray_generate_retry_backoff_s", 10.0))
        for attempt in range(max_retries + 1):
            try:
                return self.actor_rollout_wg.generate_sequences(prompts=prompts)
            except Exception as exc:
                if not self._is_retryable_generate_error(exc) or attempt >= max_retries:
                    raise
                wait_s = backoff_s * (attempt + 1)
                batch_size = prompts.batch.batch_size[0] if hasattr(prompts, "batch") else -1
                n_samples = prompts.meta_info.get("n_samples", None) if hasattr(prompts, "meta_info") else None
                print(
                    f"[WARN][Ray] generate_sequences retry {attempt + 1}/{max_retries} "
                    f"tag={tag} batch_size={batch_size} n_samples={n_samples} wait={wait_s:.1f}s "
                    f"reason={type(exc).__name__}"
                )
                time.sleep(wait_s)

    def _get_generate_rpc_chunk_size(self, tag: str) -> int:
        """Resolve RPC chunk size for generate_sequences.
        <=0 means do not split and keep the original single-call behavior.
        """
        default_size = int(self.config.trainer.get("generate_rpc_chunk_size", 0))
        if tag == "train":
            return int(self.config.trainer.get("train_generate_rpc_chunk_size", default_size))
        if tag == "validate":
            return int(self.config.trainer.get("val_generate_rpc_chunk_size", default_size))
        if tag == "save_rollouts":
            return int(self.config.trainer.get("rollout_generate_rpc_chunk_size", default_size))
        return default_size

    def _get_dp_world_size(self) -> int:
        try:
            return int(getattr(self.actor_rollout_wg, "world_size", 1))
        except Exception:
            return 1

    def _generate_sequences_in_chunks(self, prompts: DataProto, tag: str):
        """Split one long generate RPC into multiple smaller RPCs and concat outputs in-order."""
        total_size = len(prompts)
        requested_chunk_size = self._get_generate_rpc_chunk_size(tag)
        ...
        if chunk_size > 0 and dp_world_size > 1:
            ...
            if chunk_size < min_chunk_size:
                print(
                    f"[WARN][Ray] requested chunk_size={chunk_size} is too small for dp_world_size={dp_world_size}; "
                    f"bump to {min_chunk_size}"
                )
                chunk_size = min_chunk_size
            if chunk_size % dp_world_size != 0:
                aligned = ((chunk_size + dp_world_size - 1) // dp_world_size) * dp_world_size
                print(
                    f"[WARN][Ray] chunk_size={chunk_size} is not divisible by dp_world_size={dp_world_size}; "
                    f"align to {aligned}"
                )
                chunk_size = aligned

        if chunk_size <= 0 or total_size <= chunk_size:
            return self._generate_sequences_with_retry(prompts, tag=tag)

        if dp_world_size > 1 and total_size % dp_world_size != 0:
            print(
                f"[WARN][Ray] total_size={total_size} is not divisible by dp_world_size={dp_world_size}; "
                f"fallback to single-call generate to avoid NCCL mismatch"
            )
            return self._generate_sequences_with_retry(prompts, tag=tag)
        ...
        while start < total_size:
            ...
            if dp_world_size > 1 and (end - start) % dp_world_size != 0:
                end = start + ((end - start) // dp_world_size) * dp_world_size
                if end == start:
                    break
        ...
        # tail fallback: merge remaining samples into the last chunk.
        last_end = chunk_ranges[-1][1]
        if last_end < total_size:
            if dp_world_size > 1 and (total_size - chunk_ranges[-1][0]) % dp_world_size == 0:
                chunk_ranges[-1] = (chunk_ranges[-1][0], total_size)
            else:
                print(
                    f"[WARN][Ray] residual samples ({total_size - last_end}) cannot be dp-aligned; "
                    f"fallback to single-call generate for tag={tag}"
                )
                return self._generate_sequences_with_retry(prompts, tag=tag)

        num_chunks = len(chunk_ranges)
        print(
            f"[INFO][Ray] split generate_sequences tag={tag} total={total_size} "
            f"requested_chunk_size={requested_chunk_size} effective_chunk_size={chunk_size} "
            f"dp_world_size={dp_world_size} num_chunks={num_chunks}"
        )
        outputs = []
        for i, (start_idx, end_idx) in enumerate(chunk_ranges):
            prompt_chunk = prompts.slice(range(start_idx, end_idx))
            chunk_tag = f"{tag}[{i + 1}/{num_chunks}]"
            chunk_output = self._generate_sequences_with_retry(prompt_chunk, tag=chunk_tag)
            print(
                f"[INFO][Ray] {chunk_tag} done input={len(prompt_chunk)} output={len(chunk_output)}"
            )
            outputs.append(chunk_output)

        concat_output = DataProto.concat(outputs)
        if len(concat_output) != sum(len(x) for x in outputs):
            raise RuntimeError(
                f"generate chunk concat size mismatch: concat={len(concat_output)} "
                f"vs parts={sum(len(x) for x in outputs)}"
            )
        print(
            f"[INFO][Ray] split generate_sequences aggregated tag={tag} "
            f"output_total={len(concat_output)}"
        )
    ...
    def _create_dataloader(self):   # next fix
        ...
        from verl.utils.dataset.rob_dataset import StateDataset, collate_fn
        ...
        #                                     task_id = task_id,
        #                                     train_val ="train",
        #                                     pad_to=pad_to if task_id is not None else 0
        #                                     )
        ...
        if self.config.data.task_name == "aloha":
            from verl.utils.dataset.rob_dataset import AlohaDataset
            self.train_dataset = AlohaDataset()
            self.train_dataloader = BufferedDataLoader(DataLoader(dataset=self.train_dataset,
                                            batch_size=int(self.config.data.train_batch_size*self.config.data.oversample_factor),
                                            shuffle=True,
                                            drop_last=True,
            ...
            self.rollout_dataset = AlohaDataset()
            self.rollout_dataloader = DataLoader(dataset=self.rollout_dataset,
                                            batch_size=self.config.data.rollout_batch_size,
                                            collate_fn=collate_fn)
        else:
            state_path = f'./verl/utils/dataset/{self.config.data.task_name}_d0_states.pkl'
    ...
    def _save_rollouts(self, global_steps=0, rollout_epoch = 1, use_wm = False):
        ...
        for epoch_idx in range(rollout_epoch):
            for test_data in self.rollout_dataloader:
                ...
                # breakpoint()
                gen_test_batch = self._generate_sequences_in_chunks(test_batch, tag="save_rollouts")
                # breakpoint()
                test_batch = test_batch.union(gen_test_batch)
                # mask = ~test_batch.batch['drop'].squeeze()
                # test_batch = test_batch.slice(mask)
        ...
        metric_dict[f'rollout_reward/{self.config.data.task_name}'] = reward_tensor.mean().item()
        return metric_dict
    ...
    def _validate(self, global_steps=0):
        ...
        for test_data in self.val_dataloader:
            test_batch = DataProto.from_single_dict(test_data)
            # test_batch = DataProto.from_single_dict({'dumpy': torch.zeros(self.config.data.val_batch_size)})
            # test_batch = test_batch.select(
            #         batch_keys=['task_id', 'trial_id', 'drop'],
            #         non_tensor_batch_keys={"task_suite_name"},
            #         meta_info_keys={}
            #     )
            val_save_rollouts = bool(self.config.trainer.get("val_save_rollouts", False))
            test_batch.meta_info = {
                'eos_token_id': self.tokenizer.eos_token_id,
                'pad_token_id': self.tokenizer.pad_token_id,
                'recompute_log_prob': False,
                'do_sample': False,
                'return_rollouts': val_save_rollouts,
                'validate': True,
                'save_to_hdfs': val_save_rollouts,
                'rollout_base_dir': self.config.actor_rollout_ref.rollout_base_dir,  # 启用 rollout 保存路径
                'use_wm': self.config.actor_rollout_ref.wm.enable,  # 使用世界模型
                "global_steps":global_steps,
            }
            test_output_gen_batch = self._generate_sequences_in_chunks(test_batch, tag="validate")
            print(f"validation generation end, batch_size={len(test_output_gen_batch)}")
    ...
    def fit(self):
        ...
        logger = Tracking(project_name=self.config.trainer.project_name,
                          ...
                          debug = self.config.trainer.get('debug', False),
        ...
        if self.config.trainer.get('rollout_before_train', False):
            self._save_rollouts(global_steps = global_steps, rollout_epoch=self.config.trainer.get('sim_rollout_epoch', 1000), use_wm=True)
            return
        if self.val_reward_fn is not None and self.config.trainer.get('val_before_train', False):
            ...
            if self.config.trainer.get('val_only', False):
                return
        update_wm = self.config.actor_rollout_ref.wm.get('update_wm', False)
            ...
            if True:
                ...
                while len(valid_batch) < batch_size * n_samples:
                    ...
                    with Timer(name='gen', text="{name}: {seconds:.1f} seconds") as timer:
                        ...
                        gen_batch_output = self._generate_sequences_in_chunks(gen_batch, tag="train")
                ...
                # Optional reward-model timing: only record when RM path is truly enabled.
                # In WM-only runs, silently skipping this avoids misleading "reward: 0.0 seconds".
                if self.use_rm:
                    with Timer(name='reward_model', text="{name}: {seconds:.1f} seconds") as timer:
                ...
                self._maybe_print_metrics(metrics, global_steps)
    ...
    def filter(self, reward_tensor, batch, n_samples):
        ...
        Returns:
            DataProto: Filtered batch
        """
        # Print success/failure list for one prompt group (for quick debugging).
        # Example: {true,false,true,...}
        try:
            if 'complete' in batch.batch.keys():
                completes = batch.batch['complete'].detach().cpu().tolist()
                if len(completes) >= n_samples:
                    first_prompt = completes[:n_samples]
                    first_prompt_str = "{" + ",".join("true" if bool(x) else "false" for x in first_prompt) + "}"
                    print(f"Prompt[0] success list: {first_prompt_str}")
        except Exception as e:
            print(f"[WARN] failed to print Prompt[0] success list: {e}")
